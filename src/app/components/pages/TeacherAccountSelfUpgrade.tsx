...
export const TeacherAccountSelfUpgrade = () => {
    ...
    return <Container>
        <Row>
            <Col>
                <TitleAndBreadcrumb currentPageTitle={t('upgradeToATeacherAccount', 'Upgrade to a teacher account')}/>
            </Col>
        </Row>
        <Row>
            <Col className="pt-4 pb-7">
                <PageFragment fragmentId={"teacher_account_self_upgrade"}></PageFragment>
                <Button className="my-3" disabled={!emailVerified} onClick={upgrade}>{t('getATeacherAccount', 'Get a teacher account')}</Button>
                {!emailVerified && <small className="text-danger d-block">{t('yourEmailAddressIsNotVerifiedPleaseClickOnTheLinkInTheVerificationEmailToConfirmYourEmailAddressYouCan2', 'Your email address is not verified — please click on the link in the verification email to confirm your email address. You can')} <Button color="link primary-font-link" onClick={requestVerificationEmail}>{t('requestANewVerificationEmail', 'request a new verification email')}</Button> {t('ifNecessary', 'if necessary.')}</small>}
            </Col>
        </Row>
    </Container>;
};
