...
const passwordResetInformation = function(member: AppGroupMembership, passwordRequestSent: boolean) {
    let message = i18next.t('cannotSendPasswordResetRequestEmail', 'Cannot send password reset request email.');
    if (canSendPasswordResetRequest(member, passwordRequestSent)) {
        message = i18next.t('sendAPasswordResetRequestToThisUsersEmailAddress', 'Send a password reset request to this user\'s email address.');
    } else if (passwordRequestSent) {
        message = i18next.t('passwordResetRequestSent', 'Password reset request sent.');
    } else if (member.emailVerificationStatus == 'DELIVERY_FAILED') {
        message = i18next.t('passwordResetRequestCannotBeSentBecauseThisUsersAccountEmailAddressIsEitherInvalidOrNotAcceptingEmail', 'Password reset request cannot be sent because this user\'s account email address is either invalid or not accepting email.');
    }
    return message;
};
...
const confirmDeleteGroup = (dispatch: AppDispatch, deleteGroup: any, user: RegisteredUserDTO, groupToDelete: AppGroup) => {
    if (user.id === groupToDelete.ownerId) {
        if (confirm(i18next.t('areYouSureYouWantToPermanentlyDeleteTheGroup', 'Are you sure you want to permanently delete the group \'') + groupToDelete.groupName + "' and remove all associated assignments?\n\nThis action cannot be undone!")) {
            deleteGroup(groupToDelete.id as number);
        }
    } else {
        if (confirm(i18next.t('youCannotDeleteThisGroupBecauseYouAreNotTheGroupOwnerDoYouWantToRemoveYourselfAsAManagerOf', 'You cannot delete this group, because you are not the group owner.  Do you want to remove yourself as a manager of \'') + groupToDelete.groupName + "'?")) {
        ...
        <div className="pt-1 d-flex flex-fill">
            ...
            <div>
                {member.authorisedFullAccess ?
                    ...
                    <span className="not-authorised"><span className="ps-1 struck-out">{member.givenName} {member.familyName}</span> {t('notSharing', '(Not Sharing)')}</span>
                }
            </div>
            <div>
                {member.emailVerificationStatus == "DELIVERY_FAILED" &&
                    <Tooltip tipText={<><Trans i18nKey="thisUserapossAccountEmailAddressIsInvalidOrNotAcceptingEmailbrTheyWillNotBeAbleToResetTheirPasswordOrReceiveUpdateEmailsAskThemToLoginAndUpdateItOrContactUsToHelpResolveTheIssue">This user&apos;s account email address is invalid or not accepting email.<br />
                        They will not be able to reset their password or receive update emails. Ask them to login and update it, or contact us to help resolve the issue.</Trans></>} className="icon-email-status failed" />
                }
                ...
                {member.groupMembershipInformation && member.groupMembershipInformation.status == "INACTIVE" &&
                    <Tooltip className="ms-1" tipText="This user has set their status to inactive for this group. This means they will no longer see new assignments.">{t('inactiveInGroup', '(inactive in group)')}</Tooltip>
                }
            </div>
        </div>
            ...
            {["xs", "sm"].includes(deviceSize)? <>
                    ...
                    <DropdownMenu>
                        {isTeacherOrAbove(user) &&
                            <DropdownItem onClick={resetPassword} disabled={!canSendPasswordResetRequest(member, passwordRequestSent)}>
                                {!passwordRequestSent ? t('resetPassword', 'Reset password') : t('resetEmailSent', 'Reset email sent')}
                            </DropdownItem>
                        }
                        {userHasAdditionalGroupPrivileges &&
                            <DropdownItem onClick={confirmDeleteMember} aria-label={t('removeMember', 'Remove member')}>{t('remove', 'Remove')}</DropdownItem>
                        }
                    </DropdownMenu>
                </UncontrolledButtonDropdown>
            </> : <>
                ...
                {userHasAdditionalGroupPrivileges &&
                    <Tooltip tipText="Remove this user from the group." className="text-end d-none d-sm-block">
                        <Button color="link" size="sm" className="mx-2" onClick={confirmDeleteMember} aria-label={t('removeMember', 'Remove member')}>
                            {t('remove', 'Remove')}
                        </Button>
                    </Tooltip>
                }
...
const GroupEditor = ({group, allGroups, user, ...rest}: GroupEditorProps) => {
    ...
    function toggleSelfRemoval() {
        if (group) {
            ...
            updateGroup({
                updatedGroup,
                message: t('groupMemberSelfremoval', 'Group member self-removal ') + (updatedGroup.selfRemoval ? "enabled" : "disabled") + "."
            });
        }
    }
    ...
    function toggleArchived() {
        if (group) {
            ...
            updateGroup({
                updatedGroup,
                message: t('group2', 'Group ') + group.groupName + (updatedGroup.archived ? " archived" : " unarchived")
            });
        }
    }
            ...
            <div className={"d-flex flex-column gap-2"}>
                    ...
                    <Form className="form-inline" onSubmit={saveUpdatedGroup}>
                        ...
                        <InputGroup className="flex-column flex-md-row align-items-center gap-2 stackable-input-group w-100">
                            <Input
                                id="groupName"
                                length={50}
                                placeholder={t('groupName', 'Group name')} value={newGroupName}
                                onChange={e => setNewGroupName(e.target.value)} aria-label={t('groupName2', 'Group Name')} disabled={!(isUserGroupOwner || group.additionalManagerPrivileges)}
                            ...
                            <FormFeedback id={"groupNameFeedback"}>A{existingGroupWithConflictingName?.archived ? <>{t('nArchived', 'n archived')}</> : <></>} {t('groupWithThatNameAlreadyExists', 'group with that name already exists.')}</FormFeedback>
                        </InputGroup>
                    </Form>
                </div>
                ...
                <div>
                    <div className={"d-flex flex-column flex-md-row justify-content-between"}>
                        <div>
                            <p className={"fw-bold"}>{t('groupManagers', 'Group managers')}</p>
                            ...
                            <div>
                                <Button className="w-100 d-inline-block text-nowrap" color="keyline" onClick={() => dispatch(showGroupManagersModal({group, user}))}>
                                    {(isUserGroupOwner || group.additionalManagerPrivileges) ?
                                        t('valGroupManagers', '{{val}} group managers', { val: additionalManagers.length > 1 ? "Edit" : "Add" }) : `More information`
                                    }
                                </Button>
                            </div>}
                    </div>
                ...
                <div>
                    <ShowLoading until={group.members}>
                        {group.members && <div data-testid="group-members" className={"d-flex flex-column gap-3"}>
                            <div className={"d-flex flex-column flex-md-row justify-content-between"}>
                                <p className={"fw-bold"}>{t('groupMembers', 'Group members')}</p>
                            ...
                            <div>
                                <StyledCheckbox
                                    ...
                                    label={<span>{t('allowStudentsToRemoveThemselvesFromThisGroup', 'Allow students to remove themselves from this group')}</span>}
                                />
                            </div>
                            <div>{t('thisGroupHasLengthMember', 'This group has {{length}} member', { length: group.members.length })}{group.members.length != 1 ? 's' : ''}.
                                {bigGroup && !isExpanded &&
                                            <ButtonDropdown className="float-end" toggle={() => setExpanded(true)}>
                                                <DropdownToggle caret>{t('show', 'Show')}</DropdownToggle>
                                            </ButtonDropdown>
                                }
                            </div>
                ...
                {canArchive && <>
                    ...
                    <div>
                        <Button className={classNames("w-100 w-md-auto mb-2", {"mt-n3": isPhy})} color="keyline"
                            ...
                            {t('valGroup', '{{val}} group', { val: group.archived ? "Unarchive" : "Archive" })}
                        </Button>
...
export const GroupSelector = ({user, groups, allGroups, selectedGroup, setSelectedGroupId, showArchived, setShowArchived, showCreateGroup, sidebarStyle}: GroupSelectorProps) => {
    ...
    return <Card className="group-selector">
        <CardBody>
            { showCreateGroup &&
                <>
                    <Button className={"d-block w-100"} onClick={() => {dispatch(showCreateGroupModal({user}));}}>{t('createANewGroup', 'Create a new group')}</Button>
                    {siteSpecific(<div className="section-divider"/>, <hr/>)}
                </>
            }

            <div className={classNames("text-start", {"mt-3": showCreateGroup})}>
                <strong className="me-2">{t('groups2', 'Groups:')}</strong>
                <UncontrolledButtonDropdown size="sm">
            ...
            <ul className="mt-3 mt-lg-0 p-0 mb-0">
                {sortedGroups && sortedGroups.length > 0
                        ...
                        sidebarStyle
                            ? <li key={g.id} className="d-flex align-items-center group-item" data-testid={"group-item"}>
                                <StyledTabPicker
                                    ...
                                    suffix={showArchived ? {icon: "icon icon-close", action: (e) => {e.stopPropagation(); confirmDeleteGroup(dispatch, deleteGroup, user, g);}, info: t('deleteGroup', 'Delete group')} : undefined}
                                />
                            </li>
                            : <div key={g.id} className="group-item p-md-2" data-testid={"group-item"}>
                                <div className="d-flex justify-content-between align-items-center group-name-buttons">
                                    <Button title={isStaff(user) ? t('groupIdId', 'Group id: {{id}}', { id: g.id }) : undefined} color="link" data-testid={"select-group"} className="text-start px-1 py-1 flex-fill group-name" onClick={() => setSelectedGroupId(g.id)}>
                                        {g.groupName}
                                    </Button>
                                    {showArchived && (isPhy ?
                                        <button onClick={(e) => {e.stopPropagation(); confirmDeleteGroup(dispatch, deleteGroup, user, g);}}
                                            aria-label={t('deleteGroup', 'Delete group')} className="ms-1 icon-close" title={t('deleteGroup', 'Delete group')}/> :
                                        <IconButton icon={{name: "icon-bin", color: "white"}} className="action-button" affixClassName="icon-sm" aria-label={t('deleteGroup', 'Delete group')} title={t('deleteGroup', 'Delete group')} onClick={() => confirmDeleteGroup(dispatch, deleteGroup, user, g)}/>)
                                    }
                                </div>
                    ...
                    : <div className="text-center p-2">
                        {t('youHaveNo', 'You have no')} {showArchived ? "archived" : "active"} groups.
                    </div>
                }
            </ul>
        </CardBody>
...
export const Groups = ({user}: {user: RegisteredUserDTO}) => {
    ...
    const pageHelp = <span><Trans i18nKey="useThisPageToManageGroupsYouCanAddUsersToAGroupByGivingThemTheGroupCodeAndAskingThemPasteItIntoTheirAccountSettingsPageBrYouCanFindTheCodeForAnExistingGroupBySelectingTheGroupAndClickingIinviteUsersi">Use this page to manage groups. You can add users to a group by giving them the group code and asking them paste it into their account settings page.
        <br />
        You can find the code for an existing group by selecting the group and clicking <i>Invite Users</i>.</Trans></span>;
    ...
    >
        <ShowLoadingQuery query={groupQuery} defaultErrorTitle={t('errorFetchingGroups', 'Error fetching groups')}>
    ...
    >
        <ShowLoadingQuery query={groupQuery} defaultErrorTitle={t('errorFetchingGroups', 'Error fetching groups')}>
            {!isEmptyState ?
                <>
                    <p className={"mw-75"}>{t('youCanAddOtherTeachersToHelpYouManageAGroupYouCannotDirectlyAddStudentsButYouCanInviteThemToJoin', 'You can add other teachers to help you manage a group. You cannot directly add students, but you can invite them to join.')}</p>
                    <Row className="mb-7">
                        <Col lg={4}>
                ...
                :
                <div className={"mb-7"}>
                    <p className={"mw-50"}><Trans i18nKey="organiseYourStudentsIntoGroupsAndSetWorkAppropriateForEachGroupBrYouNeedAStudentGroupBeforeYouCanAssignQuizzesAndTestsInSite_title_short">Organise your students into groups and set work appropriate for each group.
                        <br />
                        You need a student group before you can assign quizzes and tests in {{ SITE_TITLE_SHORT }}.</Trans></p>
                    <Button onClick={() => void dispatch(showCreateGroupModal({user}))}>
                        {t('createAGroup', 'Create a group')}
                    </Button>
                </div>
            }
        </ShowLoadingQuery>
