...
import {QuizContentsComponent} from "../../elements/quiz/QuizContentsComponent";
...
export const QuizView = ({user}: {user: RegisteredUserDTO}) => {
    ...
    const quizRubricQuery = useGetQuizRubricQuery(quizId as string);
    const { CRUMBS } = useDynamicValues();
    const Error = useMemo(() => buildErrorComponent("Unknown Test", "There was an error loading that test.", CRUMBS.PRACTICE_TESTS), [CRUMBS.PRACTICE_TESTS]);
    const view = {
