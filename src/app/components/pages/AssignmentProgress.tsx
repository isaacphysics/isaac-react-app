...
export const ProgressDetails = ({ assignment }: { assignment: EnhancedAssignmentWithProgress }) => {
  ...
  function sortItem(props: ComponentProps<"th"> & { itemOrder: SortOrder }) {
    ...
    return (
      <th key={props.key} {...rest} className={className} onClick={clickToSelect}>
        {props.children}
        {sortArrows}
      </th>
    );
  ...
  function markQuestionClasses(studentProgress: AppAssignmentProgress, index: number) {
    ...
    return (
      isSelected(question) +
      " " +
      markClassesInternal(studentProgress, status, correctParts, incorrectParts, totalParts)
    );
  }
  ...
  return (
    <div className="assignment-progress-progress">
      <div className="progress-header">
        <strong>{studentsCorrect}</strong> of <strong>{progress.length}</strong> students have completed the gameboard{" "}
        <Link to={`/gameboards#${assignment.gameboardId}`}>{assignment.gameboard.title}</Link> correctly.
      </div>
      {progress.length > 0 && (
        <>
          <div className="progress-questions">
            ...
            <div>
              <Link to={`/questions/${selectedQuestion.id}?board=${assignment.gameboardId}`}>
                <strong>
                ...
                {sortedProgress.map((studentProgress) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`${markClasses(studentProgress, assignmentTotalQuestionParts)}${
                        fullAccess ? "" : " not-authorised"
                      }`}
                      title={`${studentProgress.user.givenName + " " + studentProgress.user.familyName}`}
                    >
                      <th className="student-name">
                        {fullAccess && pageSettings.isTeacher ? (
                          ...
                          <span>
                            {studentProgress.user.givenName} {studentProgress.user.familyName}
                          </span>
                        )}
                      </th>
                      {questions.map((q, index) => (
                        <td
                          key={q.id}
                          className={markQuestionClasses(studentProgress, index)}
                      ...
                      <th className="total-column right" title={fullAccess ? undefined : "Not Sharing"}>
                        {fullAccess
                          ? formatMark(studentProgress.tickCount, questions.length, pageSettings.formatAsPercentage)
                          : ""}
                      </th>
                    </tr>
                  );
      ...
      >
        <Button color="link" className="gameboard-title align-items-center" onClick={() => setIsExpanded(!isExpanded)}>
          <span className={classNames({ "text-muted": assignmentHasNotStarted })}>
            ...
            {assignment.dueDate && (
              <span className="gameboard-due-date">(Due:&nbsp;{formatDate(assignment.dueDate)})</span>
            )}
          </span>
        </Button>
        ...
        <div className="assignment-progress-options">
          ...
          <label>
            ...
            <input
              ...
              onChange={(e) => pageSettings.setFormatAsPercentage(e.target.checked)}
            />
          </label>
        </div>
      </div>
        ...
        <Button color="link" className="gameboard-title align-items-center" onClick={() => setIsExpanded(!isExpanded)}>
          <span>
            ...
            {isDefined(quizAssignment.dueDate) && (
              <span className="gameboard-due-date">(Due:&nbsp;{formatDate(quizAssignment.dueDate)})</span>
            )}
          </span>
        </Button>
        <div className="gameboard-links align-items-center">
...
export function AssignmentProgress({ user }: { user: RegisteredUserDTO }) {
  ...
  const pageHelp = (
    <span>{"Click on your groups to see the assignments you have set. View your students' progress by question."}</span>
  );
              ...
              {sortedGroups.length === 0 && (
                <Container className="py-5">
                  <h3 className="text-center">
                    You&apos;ll need to create a group using <Link to="/groups">Manage groups</Link> to set an
                    assignment.
                  </h3>
                </Container>
              )}
