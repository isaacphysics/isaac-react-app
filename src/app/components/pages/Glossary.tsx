...
import { isDefined, Item, NOT_FOUND, scrollVerticallyIntoView, TAG_ID, tags, useUrlHashValue } from "../../services";
...
export const Glossary = () => {
  ...
  const glossaryTerms = useMemo(() => {
    ...
    const sortedAndFilteredTerms = (
      searchText === "" ? [...(rawGlossaryTerms ?? [])] : rawGlossaryTerms?.filter((e) => e.value?.match(regex))
          ...
          <Col className="pt-2 pb-4">
            ...
            {Object.entries(glossaryTerms).map(([letter, terms]) => (
              <div
                key={letter}
                className="row pb-5"
                ref={(el: HTMLDivElement) => alphabetHeaderRefs.current.set(letter, el)}
              >
                <Col md={{ size: 1, offset: 1 }}>
                  <h2 style={{ position: "sticky", top: `calc(0px - ${ALPHABET_HEADER_OFFSET}px)` }}>{letter}</h2>
                </Col>
                <Col>
                  {terms.map((term) => (
                    <IsaacGlossaryTerm
