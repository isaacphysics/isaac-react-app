...
const isDropZone = (item: { id: UniqueIdentifier } | null) =>
  item?.id === CLOZE_ITEM_SECTION_ID || String(item?.id).slice(0, 10) === CLOZE_DROP_ZONE_ID_PREFIX;

const augmentInlineItemWithUniqueReplacementID = (idv: Immutable<ClozeItemDTO> | undefined) =>
  isDefined(idv) ? { ...idv, replacementId: `${idv?.id}|${uuid_v4()}` } : undefined;
const augmentNonSelectedItemWithReplacementID = (item: Immutable<ClozeItemDTO>) => ({
  ...item,
  replacementId: item.id,
});
const itemNotNullAndNotInAttempt =
  (currentAttempt: { items?: (Immutable<ItemDTO> | undefined)[] }) => (i: Immutable<ClozeItemDTO> | undefined) =>
  ...
  return (
    <div className={"mb-3"}>
      ...
      <Label className={"sr-only"} id={"item-section-info"}>
        To pick up an item, press space or enter. Use the up and down arrow keys to navigate between drop zones and
        items in the question. Press space or enter again to drop the item into a new position, or to swap it with
      ...
      <SortableContext items={itemIds} strategy={() => null}>
        <div
          ...
          className={`item-section rounded p-2 bg-grey ${isOverContainer ? "border border-dark" : "border-light"}`}
        >
          {items.map((item, i) => (
            <Item key={i} item={item} id={item.replacementId as string} type={"item-section"} />
          ))}
        </div>
      </SortableContext>
    </div>
...
const useAutoScroll = (
  { active, acceleration, interval }: { active: boolean; acceleration?: number; interval?: number } = { active: false },
) => {
  ...
  const autoScrollListener = useCallback((event: MouseEvent | TouchEvent) => {
    ...
    const y = (isTouchEvent(event) ? (event.touches[0] || event.changedTouches[0]).clientY : event.clientY) - baseline;
...
const IsaacClozeQuestion = ({
  ...
  const currentAttempt = useMemo(
    () => (rawCurrentAttempt ? { ...rawCurrentAttempt, items: replaceNullItems(rawCurrentAttempt.items) } : undefined),
    [rawCurrentAttempt],
  );
  ...
  const [nonSelectedItems, setNonSelectedItems] = useState<Immutable<ClozeItemDTO>[]>(
    doc.items ? [...doc.items].map(augmentNonSelectedItemWithReplacementID) : [],
  );

  const registeredDropRegionIDs = useRef<Map<string, number>>(new Map()).current;

  const [inlineDropValues, setInlineDropValues] = useState<(Immutable<ClozeItemDTO> | undefined)[]>(
  ...
  const inlineDropValueMap = useMemo<{ [p: string]: ClozeItemDTO }>(
    () =>
      Array.from(registeredDropRegionIDs.entries()).reduce(
        (dict, [dropId, i]) => Object.assign(dict, { [dropId]: inlineDropValues[i] }),
        {},
      ),
    [inlineDropValues],
  );
  ...
  useEffect(() => {
    if (isDefined(itemsCorrect)) {
      ...
      setDropZoneValidationMap(
        Array.from(registeredDropRegionIDs.entries()).reduce(
          (dict, [dropId, i]) =>
            Object.assign(dict, { [dropId]: { correct: itemsCorrect.at(i), itemId: inlineDropValueMap[dropId]?.id } }),
          {},
        ),
      );
    }
    ...
    function updateStateOnCurrentAttemptChange() {
      if (currentAttempt?.items) {
        ...
        if (!withReplacement) {
          setNonSelectedItems(
            (nsis) =>
              nsis.filter(itemNotNullAndNotInAttempt(currentAttempt)).map(augmentNonSelectedItemWithReplacementID) ||
              [],
          );
        }
      }
  ...
  const findItemById = (id: string) =>
    nonSelectedItems.find((i) => i.replacementId === id) ?? inlineDropValues.find((i) => i && i.replacementId === id);
  ...
  const onDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const isToItemSection =
        isDefined(nonSelectedItems.find((i) => i.replacementId === over?.id)) || over?.id === CLOZE_ITEM_SECTION_ID;
    ...
    (args) => {
      ...
      if (itemSection) {
        const justNonSelectedItems = args.droppableContainers.filter(
          (dc) => nonSelectedItems.findIndex((i) => i.replacementId === dc.id) !== -1,
        );
  ...
  return (
    <div className="question-content cloze-question" id={cssFriendlyQuestionPartId}>
