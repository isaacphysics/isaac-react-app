    ...
    if (isDefined(items[index].indentation)) {
      const indentedItem = {
        ...
        indentation: Math.min(
          ...
          Math.min((items[Math.max(index - 1, 0)].indentation || 0) + 1, PARSONS_MAX_INDENT),
        ),
      };
      items.splice(index, 1, indentedItem);
    }
              ...
              {(provided: DroppableProvided) => {
                return (
                  <div
                    ...
                    className={`parsons-items ${availableItems && availableItems.length > 0 ? "" : "empty"}`}
                  >
                    {availableItems &&
              ...
              {(provided: DroppableProvided) => {
                return (
                  <div
                    ...
                    ref={provided.innerRef}
                    className={classNames("parsons-items", draggedElement ? "is-dragging" : "", {
                      [`ghost-indent-${currentIndent}`]: isDefined(draggedElement) && currentIndent !== null,
                      empty: !(currentAttempt && currentAttempt.items && currentAttempt.items.length > 0),
                    })}
                  >
                    {currentAttempt &&
                      ...
                      currentAttempt.items.map((item, index) => {
                        const canDecreaseIndentation =
                          canIndent && isDefined(item?.indentation) && item.indentation > 0;
                        const canIncreaseIndentation =
                          canIndent &&
                          isDefined(item?.indentation) &&
                          index !== 0 &&
                          item.indentation <= getPreviousItemIndentation(index) &&
                                    ...
                                    {canIndent && (
                                      <div className="controls">
                                        ...
                                        <span
                                          ...
                                          role="img"
                                          onMouseUp={() => reduceIndentation(index)}
                                          aria-label={`reduce indentation ${
                                            !canDecreaseIndentation ? "(disabled)" : ""
                                          }`}
                                        >
                                          &nbsp;
                                        </span>
                                        ...
                                        <span
                                          ...
                                          role="img"
                                          onMouseUp={() => increaseIndentation(index)}
                                          aria-label={`increase indentation ${
                                            !canIncreaseIndentation ? "(disabled)" : ""
                                          }`}
                                        >
                                          &nbsp;
                                        </span>
