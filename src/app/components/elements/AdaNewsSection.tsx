...
export const AdaNewsSection = ({isHomepage}: {isHomepage?: boolean}) => {
    ...
    return ((news && news.length > 0) || showNewsletterPrompts) && <>
        <h2 className={classNames({"font-size-1-75 mb-4": isHomepage})}>{t('tipsToolsSupport', 'Tips, tools & support')}</h2>
        {news && news.length > 0 &&
            <>
                ...
                <div className={"mt-4 mt-lg-5 w-100 text-center"}>
                    <Button href={"/news"} color={"link"}><h4 className={"mb-0"}>{t('seeMore', 'See more')}</h4></Button>
                </div>
            </>}
        {showNewsletterPrompts &&
            <Row xs={12} className={classNames({"pt-4": !isHomepage}, "mt-7")}>
                <IconCard
                    card={{
                        title: t('stayUpdated', 'Stay updated'),
                        icon: {name: "icon-mail", color: "tertiary"},
                        bodyText: t('getFreshTeachingIdeasStudentStudyTipsAndDevelopmentNewsAboutOurToolsAndResourcesDeliveredStraightToYourInbox', 'Get fresh teaching ideas, student study tips, and development news about our tools and resources delivered straight to your inbox.'),
                        clickUrl: "/account#notifications",
                        buttonText: t('getTipsAndUpdates', 'Get tips and updates'),
