...
export const IconCard = ({card, ...props}: IconCardProps) => {
    ...
    return <Container {...props} className={classNames("icon-card-container px-3 my-3", props?.className ?? "")}>
        <Card className={"icon-card border-0"} tag={buttonStyle === "card" ? Link : Card} to={clickUrl}>
            ...
            <div className={classNames("d-flex flex-column h-100 icon-card-main-content", {"pb-4" : !clickUrl || buttonStyle === "card"})}>
                ...
                {clickUrl && buttonStyle !== "card" && <CardFooter className={"border-top-0 p-4"}>
                    <Button className={classNames({"d-flex align-items-center" : buttonStyle === "link"}, {"external-link": !isAppLink(clickUrl)})} disabled={disabled} outline={buttonStyle === "outline"} color={buttonStyle === "link" ? "link" : "secondary"} tag={Link} to={clickUrl}>
                        {buttonText || "See more"}
                    </Button>
                </CardFooter>}
            </div>
