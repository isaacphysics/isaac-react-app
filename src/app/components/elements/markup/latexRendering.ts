  ...
  while ((match = start.exec(html)) !== null) {
    ...
    if (search.just) {
      ...
      if (search.matched && match) {
        ...
        let macrosToUse =
          booleanNotation === BOOLEAN_NOTATION.ENG ? KatexMacrosWithEngineeringBool : KatexMacrosWithMathsBool;
        macrosToUse = {
          ...macrosToUse,
          "\\ref": (context: { consumeArgs: (n: number) => { text: string }[][] }) => {
        ...
        if (screenReaderText) {
          ...
          const katexMathML = katex
            .renderToString(latexMunged.replace(dropZoneRegex, "clickable drop zone"), {
              ...katexOptions,
              output: "mathml",
            })
