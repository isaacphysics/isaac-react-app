      ...
      <Row className="board-card" data-testid={"my-assignment"}>
        ...
        <Col xs={8} md={3} lg={4} className="pl-lg-5">
          ...
          {isDefined(assignment.assignerSummary) && (
            <p>
              <strong>By:</strong> {extractTeacherName(assignment.assignerSummary)}
            </p>
          )}
        </Col>
        <Col xs={7} md={5} className="mt-sm-2">
          <p className="mb-0">
            <strong>Questions:</strong> {assignment.gameboard?.contents?.length || "0"}
          </p>
          {isDefined(topics) && topics.length > 0 && (
            <p className="mb-0">
              <strong>{topics.length === 1 ? "Topic" : "Topics"}:</strong> {topics.join(", ")}
            </p>
          )}
          {boardStagesAndDifficulties.length > 0 && (
            <div className="mb-0">
              <table className="w-100">
                <thead>
                  <tr>
                    <th className="w-50">{`Stage${boardStagesAndDifficulties.length > 1 ? "s" : ""}:`}</th>
                    <th className="w-50">
                ...
                <tbody>
                  {boardStagesAndDifficulties.map(([stage, difficulties]) => (
                    <tr key={stage}>
                      <td className="w-50 align-baseline">{stageLabelMap[stage]}:</td>
                      <td className="w-50 pl-1">{difficulties.map((d) => difficultyShortLabelMap[d]).join(", ")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <Col xs={5} md={2} className="mt-sm-2 text-right">
          <Link to={`/gameboards#${assignment.gameboardId}`}>View Assignment</Link>
          {isDefined(assignment.dueDate) &&
            isDefined(assignment.gameboard) &&
            now > midnightOf(assignment.dueDate) &&
            assignment.gameboard.percentageCompleted !== 100 && (
              <p>
                <strong className="overdue">Overdue:</strong> {formatDate(assignment.dueDate)}
              </p>
            )}
        </Col>
      </Row>
