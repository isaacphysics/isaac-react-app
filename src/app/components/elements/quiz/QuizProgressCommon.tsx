          ...
          {sections.map((section) => {
            ...
            return questionsInSection(section)
              .map((question) => {
                ...
                return (
                  <td key={question.id} className={markQuestionClasses(row, mark, outOf)}>
                    {icon}
                  </td>
                );
              })
      ...
      <table className="progress-table border">
        <tbody>
          <tr className="bg-white">
            ...
            {sections.map((section) => (
              <th key={section.id} colSpan={questionsInSection(section).length} className="border font-weight-bold">
                {section.title}
              </th>
            ))}
          ...
          {assignment.userFeedback?.map((row) => <ResultRow key={row.user?.id} row={row} assignment={assignment} />)}
        </tbody>
      </table>
    </div>
  );
