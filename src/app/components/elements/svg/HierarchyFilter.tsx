...
import { Item, SUBJECT_SPECIFIC_CHILDREN_MAP, TAG_ID, TAG_LEVEL, tags } from "../../../services";
...
export const HierarchyFilterTreeContents = ({tier, index, choices, selections, questionFinderFilter, root, setSelections}: Omit<HierarchyFilterProps, 'className'>) => {
    ...
    const getIsDisabled = (choice: Item<TAG_ID>) => (pageContext?.subject && pageContext?.stage?.[0] && disabledSubjectStageTopics[pageContext?.subject]?.[pageContext?.stage?.[0]]?.includes(choice.value)) || false;
    ...
    return <>
        {choices[tier] && choices[tier][index] && 
            choices[tier][index].sort((choiceA, choiceB) => Number(getIsDisabled(choiceA)) - Number(getIsDisabled(choiceB))).map((choice, i) => {
                ...
                const isDisabled = getIsDisabled(choice);
