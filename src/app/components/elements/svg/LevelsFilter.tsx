    ...
    >
      ...
      <g transform={`translate(1,1)`}>
        {levelOptions
          ...
          .map((level, i) => (
            <g key={level.value} transform={`translate(${i * 2 * (hexagon.halfWidth + hexagon.padding)}, 0)`}>
              <Hexagon {...hexagon} className={`hex level active`} />
              <foreignObject width={hexagon.halfWidth * 2} height={hexagon.quarterHeight * 4}>
                <div className={`hexagon-level-title active  hex-level-${level.value}`}>{level.label}</div>
              </foreignObject>
            </g>
          ))}
      </g>
...
export function LevelsFilterHexagonal({ levelOptions, levels, setLevels }: LevelsFilterProps) {
  ...
  return (
    <svg width="100%" height={`${2 * focusPadding + 7 * hexagon.quarterHeight + 2 * hexagon.padding}px`}>
              ...
              {levelOptionsRow.map((levelOption, j) => {
                ...
                return (
                  <g key={i + "-" + j} transform={`translate(${j * 2 * (hexagon.halfWidth + hexagon.padding)}, 0)`}>
                    ...
                    <foreignObject width={hexagon.halfWidth * 2} height={hexagon.quarterHeight * 4}>
                      <div
                        className={`hexagon-level-title ${isSelected ? "active" : ""} hex-level-${levelOption.value}`}
                      >
                        {levelOption.label}
                      </div>
                    </foreignObject>
                    ...
                    >
                      <title>
                        {`${isSelected ? "Remove" : "Add"} level ${levelOption.label} ${
                          isSelected ? "from" : "to"
