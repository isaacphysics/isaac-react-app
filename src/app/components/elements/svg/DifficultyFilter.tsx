...
export function DifficultyFilter({ difficultyOptions, difficulties, setDifficulties }: DifficultyFilterProps) {
  ...
  return (
    <Container
      ...
      style={{ width: 2 * focusPadding + 6 * (hexagon.halfWidth + 2 * hexagon.padding) }}
    >
      <span>Practice</span>
      <svg
        width={`${2 * focusPadding + 6 * (hexagon.halfWidth + hexagon.padding)}`}
        height={`${2 * focusPadding + 4 * hexagon.quarterHeight + 2 * hexagon.padding}px`}
      >
            ...
            {practiceOptionsRow.map((difficultyOption, j) => {
              ...
              return (
                <g
                  ...
                  transform={`translate(${j * 2 * (hexagon.halfWidth + hexagon.padding)}, 0)`}
                >
                  ...
                  <foreignObject width={hexagon.halfWidth * 2} height={hexagon.quarterHeight * 4}>
                    <div
                      className={`difficulty-title ${isSelected ? "active" : ""} difficulty-${difficultyOption.value} `}
                    >
                  ...
                  >
                    <title>
                      {`${isSelected ? "Remove" : "Add"} difficulty ${difficultyOption.label} ${
                        isSelected ? "from" : "to"
                  ...
                  {[1, 0, -1].slice(0, getDifficultyLevel(difficultyOption.value)).map((n) => (
                    <g
                      ...
                      transform={`translate(${
                        2 * hexagon.halfWidth - (hexagon.padding + 2 * miniHexagon.halfWidth + miniHexagon.padding)
                      }, ${
                        2 * hexagon.quarterHeight -
                        2 * miniHexagon.quarterHeight +
                        n * (4 * miniHexagon.quarterHeight + miniHexagon.padding)
                      })`}
                    >
                      <Hexagon
                        {...miniHexagon}
                        className={`hex practice difficulty mini ${isSelected ? "" : "active"}`}
                      />
                    </g>
                  ))}
                </g>
      ...
      >
        <g transform={`translate(${focusPadding + square.padding},${focusPadding})`}>
          {challengeOptionsRow.map((difficultyOption, j) => {
            ...
            return (
              <g key={difficultyOption.value} transform={`translate(${j * (square.width + 2 * square.padding)}, 0)`}>
                ...
                {[0, 1, 2].slice(0, getDifficultyLevel(difficultyOption.value)).map((n) => (
                  <g
                    key={`${difficultyOption.value}-${n}`}
                    transform={`translate(${square.width - (square.padding + miniSquare.width + miniSquare.padding)}, ${
                      square.height +
                      square.padding -
                      miniSquare.padding -
                  ...
                  >
                    <Rectangle
                      {...miniSquare}
                      className={`square challenge difficulty mini ${isSelected ? "" : "active"}`}
                    />
                  </g>
                ))}
              </g>
