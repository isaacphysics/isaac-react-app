...
export const ContentSummaryListGroupItem = ({
  ...
  const hierarchyTags = tags.getByIdsAsHierarchy((item.tags || []) as TAG_ID[]).filter((t, i) => i !== 0); // CS always has Computer Science at the top level
  ...
  switch (item.type) {
    ...
    case DOCUMENT_TYPE.TOPIC_SUMMARY:
      linkDestination = `/${documentTypePathPrefix[DOCUMENT_TYPE.TOPIC_SUMMARY]}/${item.id?.slice(
        "topic_summary_".length,
      )}`;
  ...
  return (
    ...
    >
      <Link to={{ pathname: linkDestination, search: search, hash: hash }}>
        <span className="content-summary-link-title align-self-center" role="img" aria-label={iconLabel}>
          {icon}
        </span>
        <div className="d-md-flex flex-fill">
          <div className={"align-self-center " + titleClasses}>
            <div className="d-flex">
              ...
              {typeLabel && (
                <span className={"small text-muted align-self-end d-none d-md-inline ml-2 mb-1"}>({typeLabel})</span>
              )}
            </div>
