...
export function FastTrackProgress({ doc, search }: { doc: IsaacFastTrackQuestionPageDTO; search: string }) {
    ...
    for (const conceptLevelName of conceptLevels) {
      result[conceptLevelName] = unorderedConceptQuestions[conceptLevelName]
        ...
        .sort((a: { id?: string }, b: { id?: string }) =>
          a.id === b.id ? 0 : a.id === undefined || (b.id !== undefined && a.id > b.id) ? 1 : -1,
        );
    }
    return result;
  }
  ...
  function evaluateProgress(unorderedConceptQuestions: ConceptLevelQuestions, questionHistory: string[]): Progress {
          ...
          augmentQuestion(
            {
              ...
              questionPartStates:
                pageQuestionParts?.map((qp) =>
                  qp.bestAttempt ? (qp.bestAttempt.correct ? "CORRECT" : "INCORRECT") : "NOT_ATTEMPTED",
                ) || [],
            },
    ...
    if (currentlyWorkingOn.isConcept) {
      const upperAndLowerConceptQuestions: Map<QuestionLevel, GameboardItem[]> = new Map([
      ...
      upperAndLowerConceptQuestions.forEach((conceptQuestionsOfType, conceptQuestionType) => {
        conceptQuestionsOfType.forEach((question, index) => {
          if (question.id === currentlyWorkingOn.id) {
            ...
            progress.questions[conceptQuestionType].push(
              augmentQuestion(question, gameboard?.id as string, questionHistory, index),
            );
          }
        });
      });
      ...
      <g
        ...
        transform={
          ...
          (3 * hexagon.quarterHeight +
            hexagon.padding +
            connectionRowIndex * (6 * hexagon.quarterHeight + 2 * hexagon.padding)) +
          ")"
        }
      >
          ...
          <div className="d-none d-lg-block">
            ...
            {currentlyWorkingOn.isConcept && (
              <h4 className="mt-lg-1 mt-xl-3">
                <Markup encoding={"latex"}>{`${currentlyWorkingOn.title} Practice`}</Markup>
              </h4>
            )}
          </div>
        </RS.Col>
          ...
          <svg id="ft-progress" width="100%" height={progressBarHeight}>
            <g id="progress-bar-padding" transform={`translate(${progressBarPadding}, ${progressBarPadding})`}>
              ...
              <g id="question-hexagons">
                ...
                {progress.questions.upper.length && createQuestionRow(progress.questions.upper, "upper", 1)}
                {progress.questions.lower.length && createQuestionRow(progress.questions.lower, "lower", 2)}
              </g>
            </g>
          </svg>
        </RS.Col>
