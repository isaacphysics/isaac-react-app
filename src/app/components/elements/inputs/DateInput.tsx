...
export const DateInput = (props: DateInputProps) => {
  ...
  const inputGroupClasses = ["date-input", props.valid && "is-valid", props.invalid && "is-invalid"]
    .filter((x) => !!x)
    .join(" ");
  ...
  const values: {
    [what: string]: { get: () => number | undefined; set: (to: number | undefined) => void; reset: () => void };
  } = {
    ...
    <React.Fragment>
      <InputGroup id={props.id} {...controlPropsWithValidationStripped} className={inputGroupClasses}>
        ...
        >
          ...
          {range(1, Math.max(lastInMonth(), values.day.get() || 0) + 1).map((day) => (
        ...
        {(props.noClear === undefined || !props.noClear) && (
          <Button
            ...
            className="mx-1"
            aria-label={`Clear date${props.labelSuffix ? props.labelSuffix : ""}`}
            onClick={clear}
          />
        )}
      </InputGroup>
