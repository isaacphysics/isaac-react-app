...
export function generateSingleLetterMenuItem(letter: string, label?: string): MenuItemProps {
  return {
    ...
    menu: { label: label || letter, texLabel: true, className: `symbol-${letter}` },
  };
}
...
export function generateLogicFunctionsItems(syntax: LogicSyntax = "logic"): MenuItemProps[] {
  ...
  return [
    ...
    { type: "LogicNot", properties: {}, menu: { label: labels[syntax]["not"], texLabel: true, className: "not" } },
    {
      ...
      menu: { label: labels[syntax]["equiv"], texLabel: true, className: "equiv" },
    },
    {
      type: "LogicLiteral",
      properties: { value: true },
      menu: { label: labels[syntax]["True"], texLabel: true, className: "true" },
    },
    {
      type: "LogicLiteral",
      properties: { value: false },
      menu: { label: labels[syntax]["False"], texLabel: true, className: "false" },
    },
    {
      type: "Brackets",
...
export function generateMathsBasicFunctionsItems(): MenuItemProps[] {
  return [
    ...
    },
    {
      type: "Relation",
      properties: { relation: "<=" },
      menu: { label: "\\leq", texLabel: true, className: "relation less-or-equal" },
    },
    {
      type: "Relation",
      properties: { relation: ">=" },
      menu: { label: "\\geq", texLabel: true, className: "relation greater-or-equal" },
    },
  ];
}
...
export function generateMathsLogFunctionItem(name: string): MenuItemProps {
  if (name === "ln") {
    return {
      type: "Fn",
      properties: { name: "ln", innerSuperscript: false, allowSubscript: false },
    ...
    return {
      type: "Fn",
      properties: { name: "log", innerSuperscript: false, allowSubscript: true },
...
export function generateMathsDefaultDerivativesItems(availableSymbols: string[] | undefined): MenuItemProps[] {
  ...
  if (!availableSymbols || availableSymbols.length === 0) {
    const derivativeItem: MenuItemProps = {
      type: "Derivative",
      properties: {},
      menu: { label: "\\frac{\\mathrm{d}\\ \\cdot}{\\mathrm{d}\\ \\cdots}", texLabel: true, className: "derivative" },
    };
    ...
    return [
      {
        type: "Differential",
        properties: { letter: "d" },
        menu: { label: "\\mathrm{d}^{\\circ}\\circ", texLabel: true, className: "differential-d" },
      },
      {
        type: "Differential",
        properties: { letter: "∆" },
        menu: { label: "\\mathrm{\\Delta}^{\\circ}\\circ", texLabel: true, className: "differential-upper-delta" },
      },
      {
        type: "Differential",
        properties: { letter: "δ" },
        menu: { label: "\\mathrm{\\delta}^{\\circ}\\circ", texLabel: true, className: "differential-lower-delta" },
      },
      derivativeItem,
    ];
  }
...
export function generateMathsDerivativeAndLetters(symbol: string): {
  ...
  if (isDefined(GREEK_LETTERS_MAP[top]) || /^[a-zA-Z]$/.test(top)) {
    ...
    letters.push(
      generateSingleLetterMenuItem(GREEK_LETTERS_MAP[top] || top, GREEK_LETTERS_MAP[top] ? "\\" + top : top),
    );
  }
  ...
  const derivativeObject: MenuItemProps = {
    type: "Derivative",
    properties: {},
    menu: { label: texLabel, texLabel: true, fontSize: "1.5em", className: "derivative" },
  };
  const numerator = {
    type: "Differential",
    properties: { letter: "d" },
  ...
  if (isDefined(GREEK_LETTERS_MAP[top]) || /^[a-zA-Z]$/.test(top)) {
    ...
    const compoundNumerator = {
      ...
      children:
        derivativeOrder > 1
          ? { argument, order: { type: "Num", properties: { significand: `${derivativeOrder}` } } }
          : { argument },
    };
    ...
    const compoundDerivativeObject: MenuItemProps = {
      type: "Derivative",
      properties: {},
      menu: { label: compoundTexLabel, texLabel: true, fontSize: "1.5em", className: "derivative" },
    };
      ...
      letters: differentialArgument
        ? [
            generateSingleLetterMenuItem(
              ...
              GREEK_LETTERS_MAP[differentialArgument] ? "\\" + differentialArgument : differentialArgument,
            ),
          ]
        : null,
    };
...
export function setupAndTeardownDocStyleAndListeners({
  ...
  onTouchStart,
  inequalityModalRef,
}: InequalityHandlers & { inequalityModalRef: React.RefObject<HTMLDivElement> }) {
    ...
    return [
      {
        ...
        letters: uniqWith([...baseItems.letters, ...customMenuItems.letters], (a, b) =>
          isEqual(a, b),
...
export function handleMoveCallback(
  { previousCursor, movingMenuBar, movingMenuItem, potentialSymbolSpec, sketch }: HandlerArgs,
