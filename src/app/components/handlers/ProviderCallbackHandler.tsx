...
export const ProviderCallbackHandler = () => {
    ...
    const handleProviderCallbackOnLoad = useCallback(() => {
        if (provider) {
    ...
    useEffect(() => {
        // avoid strict mode issues via event handler; handleProviderCallback *must* be called only once
        window.addEventListener('load', handleProviderCallbackOnLoad);
        return () => window.removeEventListener('load', handleProviderCallbackOnLoad);
    }, [handleProviderCallbackOnLoad]);
