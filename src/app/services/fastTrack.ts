...
function trySupportingQuestion(
  ...
  return {
    ...
    title: `Try more questions of a similar difficulty${
      supportingQuestion.title ? " on " + supportingQuestion.title.toLowerCase() : ""
    }`,
...
export function useFastTrackInformation(
  ...
  const { board, questionHistory: questionHistoryUrl }: { board?: string; questionHistory?: string } =
...
export function determineFastTrackSecondaryAction(questionPart: FastTrackPageProperties) {
  if (questionPart.page) {
    if (!questionPart.correct) {
      const relatedUnansweredEasierQuestions = getRelatedUnansweredEasierQuestions(
        questionPart.doc,
    ...
    const relatedUnansweredSupportingQuestions = getRelatedUnansweredSupportingQuestions(
      questionPart.doc,
      questionPart.page.level || 0,
    );
