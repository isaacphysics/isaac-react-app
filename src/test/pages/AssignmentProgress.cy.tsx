...
describe('Assignment progress', () => {
    it.skip('Groups listing should have no visual regressions', () => {
        cy.mountWithStoreAndRouter(<AssignmentProgressWithRoute />, [PATHS.ASSIGNMENT_PROGRESS]);
        cy.get('[data-testid="loading"]').should('not.exist');
        cy.matchImage();
    });

    it.skip('Group overview (assignment view) should have no visual regressions', () => {
    ...
    it.skip('Group overview (test view) should have no visual regressions', () => {
        cy.mountWithStoreAndRouter(<AssignmentProgressWithRoute />, [`${PATHS.ASSIGNMENT_PROGRESS}/group/2`]);
        cy.get('button').contains("Tests").click();
        cy.matchImage();
    });

    it.skip('Individual assignment view (Group Overview tab) should have no visual regressions', () => {
    ...
    it.skip('Individual assignment view (Detailed Marks tab) should have no visual regressions', () => {
        cy.mountWithStoreAndRouter(<AssignmentProgressWithRoute />, [`${PATHS.ASSIGNMENT_PROGRESS}/40`]);
